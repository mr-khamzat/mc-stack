...
(function(){{
  ...
  var tx=0,ty=0,sc=1,drag=false,sx=0,sy=0,stx=0,sty=0;
  // Initial fit
  function fit(){{
  ...
  wrap.addEventListener('touchstart',function(e){{
    ...
    if(e.touches.length===1){{
      var t=e.touches[0]; sx=t.clientX; sy=t.clientY; stx=tx; sty=ty; drag=true;
    }}
  ...
  wrap.addEventListener('touchmove',function(e){{
    ...
    if(drag&&e.touches.length===1){{
      var t=e.touches[0]; tx=stx+(t.clientX-sx); ty=sty+(t.clientY-sy); applyT();
    }}
  ...
  wrap.addEventListener('touchend',function(){{drag=false;t0=null;t1=null;}});
  // Node click
  ...
  var secs=60;
  function tick(){{badge.textContent='🔄 обновление через '+secs+'с';secs--;if(secs<0){{location.reload(true);}}}}
