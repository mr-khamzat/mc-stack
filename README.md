...
| `/ai` | Чат с Claude AI |
| `/sslcheck` | Проверить SSL сертификаты вручную |
| `/link` | Открыть Mini App в браузере (прямая ссылка) |
| `/cancel` | Отменить текущее действие |
| `/users` | Управление пользователями |
| `/backup` | Бэкап конфига |

> **💡 Команда `/ai`** требует установленного [Claude CLI](https://claude.ai/download) на сервере (`/root/.local/bin/claude`). Без него команда недоступна. Переменная `ANTHROPIC_API_KEY` в `.env` не используется для бота.
...
- 💬 **Чат** — текстовые сообщения, фото и голосовые между пользователями
- 📸 **Фото** — общая галерея для членов семьи
- 🔔 **Напоминания** — установить напоминание с push-уведомлением
- 👤 **Распознавание лиц** — история идентификаций Frigate (если настроен)
...
# ⚠️ Если FRIGATE_URL не задан — функции камер (живой поток, события, распознавание лиц) недоступны.
...
> **Примечание:** При первом запуске бот автоматически создаёт:
> `ha_bot.db` (база данных), `backups/` (папка бэкапов), `vapid_private.pem` + `vapid_public.txt` (ключи Web Push), `photos/` и `voices/` (папки медиа).
